...
const TargetCursor: React.FC<TargetCursorProps> = ({
  ...
  useEffect(() => {
    const containerEl = containerSelector ? document.querySelector<HTMLElement>(containerSelector) : document.body;
    if (isMobile || !cursorRef.current || !containerEl) return;

    const originalCursor = containerEl.style.cursor;
    ...
    gsap.set(cursor, { autoAlpha: isInsideContainer ? 1 : 0 });
    ...
    return () => {
      ...
      spinTl.current?.kill();
      containerEl.style.cursor = originalCursor;
      isActiveRef.current = false;
      targetCornerPositionsRef.current = null;
      activeStrengthRef.current.current = 0;
    };
  }, [targetSelector, spinDuration, moveCursor, constants, hideDefaultCursor, isMobile, hoverDuration, parallaxOn, containerSelector]);
